...
export default function LoginPage() {
  ...
  const enter = async () => {
    ...
    setBusy(true); setErr(null); setNeedConfirm(false);
    const { error } = await sb.auth.signInWithPassword({ email, password: pw });
    setBusy(false);
    if (error) {
      if (/confirm/i.test(error.message)) {
        setNeedConfirm(true);
        setErr("החשבון עדיין לא אומת. פתחו את מייל האישור שנשלח אליכם ולחצו על הקישור.");
      } else if (/invalid|credential/i.test(error.message)) {
        setErr("אימייל או סיסמה שגויים. אפשר לאפס סיסמה בקישור למטה.");
      } else {
        setErr("ההתחברות נכשלה: " + error.message);
      }
      return;
    }
    router.push("/welcome");
  ...
  const resend = async () => {
    ...
    if (!sb || !email) return;
    setBusy(true);
    await sb.auth.resend({ type: "signup", email, options: { emailRedirectTo: `${location.origin}/welcome` } });
    setBusy(false);
    setNeedConfirm(false);
    setErr("שלחנו מייל אישור חדש. בדקו גם בתיקיית הספאם.");
  };
    ...
    <div className="view active" id="auth">
      ...
      <div className="auth-card">
        ...
        <div className="card pad" style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {needConfirm && (
            <button className="btn ghost sm" onClick={resend} style={{ alignSelf: "flex-start" }}>שליחת מייל אישור מחדש</button>
          )}
